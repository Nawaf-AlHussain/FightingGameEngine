...
static AssignmentReturnValue* enemyFunction(DreamMugenAssignment** tIndexAssignment, DreamPlayer* tPlayer, int* tIsStatic);
static AssignmentReturnValue* partnerFunction(DreamMugenAssignment** tIndexAssignment, DreamPlayer* tPlayer, int* tIsStatic);
...
static DreamPlayer* getRegularPlayerFromFirstVectorPartOrNullIfNonexistant(DreamMugenAssignment** a, DreamPlayer* tPlayer, int* tIsStatic) {
        ...
        if ((*a)->mType == MUGEN_ASSIGNMENT_TYPE_RAW_VARIABLE)
        {
                ...
                else if (arrayVar->mFunc == enemyFunction) {
                        strcpy(text, "enemy");
                }
                else if (arrayVar->mFunc == partnerFunction) {
                        strcpy(text, "partner");
                }
        ...
        else if (!strcmp("partner", text)) {
                // Phase 1.2 — partner(n) redirection. In 1v1 there are no
                // partners; returns NULL which downstream code already handles
                // gracefully (Cond/IfElse will short-circuit to the else branch).
                return getPlayerPartnerByIndex(tPlayer, id);
        }
...
static AssignmentReturnValue* airJumpCountFunction(DreamPlayer* tPlayer) { return makeNumberAssignmentReturn(getPlayerAirJumpCounter(tPlayer)); }
...
// Returns 1 if the player has any HitOverride slot currently active (would
// catch the next hit instead of entering the gethit state). Used by armor and
// counter characters (Akuma, Geese, KoldSpidey) to gate hit-reaction states.
static AssignmentReturnValue* hitOverriddenFunction(DreamPlayer* tPlayer) { return makeBooleanAssignmentReturn(isPlayerHitOverrideActive(tPlayer)); }
...
static AssignmentReturnValue* numEnemyFunction(DreamPlayer* tPlayer) { return makeNumberAssignmentReturn(getPlayerEnemyCount(tPlayer)); }
...
static AssignmentReturnValue* numPartnerFunction(DreamPlayer* tPlayer) { return makeNumberAssignmentReturn(getPlayerPartnerCount(tPlayer)); }
...
// These cover the most common remaining MUGEN 1.1 sub-keys that modern
// characters query. The Ikemen-extension sub-keys (dizzypoints, guardpoints,
// redlife, score) return 0 because Dolmexica doesn't yet track those meters;
// they will return correct values once the score/dizzy/guardpoints subsystem
// is added (out of Phase 1 scope).
static AssignmentReturnValue* getHitVarRedirectIdFunction(DreamPlayer* tPlayer) { return makeNumberAssignmentReturn(getPlayerLastAttackerID(tPlayer)); }
static AssignmentReturnValue* getHitVarPlayerIdFunction(DreamPlayer* tPlayer) { return makeNumberAssignmentReturn(getPlayerLastAttackerID(tPlayer)); }
static AssignmentReturnValue* getHitVarBindtimeFunction(DreamPlayer* tPlayer) { return makeNumberAssignmentReturn(getPlayerRemainingBindTime(tPlayer)); }
static AssignmentReturnValue* getHitVarAttrFunction(DreamPlayer* /*tPlayer*/) { return makeNumberAssignmentReturn(0); } // bitfield — full impl needs flag parsing
static AssignmentReturnValue* getHitVarDizzypointsFunction(DreamPlayer* /*tPlayer*/) { return makeNumberAssignmentReturn(0); } // Ikemen ext
static AssignmentReturnValue* getHitVarGuardpointsFunction(DreamPlayer* /*tPlayer*/) { return makeNumberAssignmentReturn(0); } // Ikemen ext
static AssignmentReturnValue* getHitVarRedlifeFunction(DreamPlayer* /*tPlayer*/) { return makeNumberAssignmentReturn(0); } // Ikemen ext
static AssignmentReturnValue* getHitVarScoreFunction(DreamPlayer* /*tPlayer*/) { return makeNumberAssignmentReturn(0); } // Ikemen ext

static void setupVariableAssignments() {
        gVariableHandler.mVariables.clear();
        gVariableHandler.mVariables["ailevel"] = aiLevelFunction;
        gVariableHandler.mVariables["airjumpcount"] = airJumpCountFunction;
        ...
        gVariableHandler.mVariables["hitoverridden"] = hitOverriddenFunction;
        ...
        // Phase 1.4 — additional gethitvar sub-keys.
        gVariableHandler.mVariables["gethitvar(redirectid)"] = getHitVarRedirectIdFunction;
        gVariableHandler.mVariables["gethitvar(redirectId)"] = getHitVarRedirectIdFunction;
        gVariableHandler.mVariables["gethitvar(playerid)"] = getHitVarPlayerIdFunction;
        gVariableHandler.mVariables["gethitvar(id)"] = getHitVarPlayerIdFunction;
        gVariableHandler.mVariables["gethitvar(bindtime)"] = getHitVarBindtimeFunction;
        gVariableHandler.mVariables["gethitvar(attr)"] = getHitVarAttrFunction;
        gVariableHandler.mVariables["gethitvar(dizzypoints)"] = getHitVarDizzypointsFunction;
        gVariableHandler.mVariables["gethitvar(guardpoints)"] = getHitVarGuardpointsFunction;
        gVariableHandler.mVariables["gethitvar(redlife)"] = getHitVarRedlifeFunction;
        gVariableHandler.mVariables["gethitvar(score)"] = getHitVarScoreFunction;
}
...
// inputtime(B) returns the number of ticks since the player last pressed
// button B. Ikemen semantics: positive value means currently held (value =
// frames held); negative means released (|value| = frames since release).
// Used by charge characters (Guile Sonic Boom: inputtime(B) > 30 && ...).
static AssignmentReturnValue* evaluateInputTimeArrayAssignment(AssignmentReturnValue* tIndex, DreamPlayer* tPlayer, int* tIsStatic) {
        string buttonName;
        convertAssignmentReturnToString(buttonName, tIndex);
        *tIsStatic = 0;

        int controllerID = tPlayer->mControllerID;
        int32_t val = getDreamInputButtonBufferTime(controllerID, buttonName.c_str());
        return makeNumberAssignmentReturn(val);
}
...
// Phase 1.3 — inputtime trigger. Takes a single button-name string argument
...
static AssignmentReturnValue* enemyFunction(DreamMugenAssignment** tIndexAssignment, DreamPlayer* tPlayer, int* tIsStatic) { return evaluateTargetArrayAssignment(evaluateAssignmentDependency(tIndexAssignment, tPlayer, tIsStatic), "enemy"); }
// Phase 1.2 — partner(n) redirection. Same pattern as enemy(n): evaluate the
// index, then defer to evaluateTargetArrayAssignment with the "partner" tag.
// The actual lookup happens in getRegularPlayerFromFirstVectorPartOrNullIfNonexistant.
static AssignmentReturnValue* partnerFunction(DreamMugenAssignment** tIndexAssignment, DreamPlayer* tPlayer, int* tIsStatic) { return evaluateTargetArrayAssignment(evaluateAssignmentDependency(tIndexAssignment, tPlayer, tIsStatic), "partner"); }
...
// Phase 1.6 — isasserted(flag) trigger. Takes a string argument (flag name)
// and returns 1 if that AssertSpecial flag is currently active for the player.
static AssignmentReturnValue* isAssertedFunction(DreamMugenAssignment** tIndexAssignment, DreamPlayer* tPlayer, int* tIsStatic) {
        AssignmentReturnValue* arg = evaluateAssignmentDependency(tIndexAssignment, tPlayer, tIsStatic);
        string flagName;
        convertAssignmentReturnToString(flagName, arg);
        *tIsStatic = 0;
        return makeBooleanAssignmentReturn(isPlayerAsserted(tPlayer, flagName.c_str()));
}
...
static void setupArrayAssignments() {
        ...
        gVariableHandler.mArrays["enemy"] = enemyFunction;
        gVariableHandler.mArrays["partner"] = partnerFunction;
        gVariableHandler.mArrays["isasserted"] = isAssertedFunction;
