...
// Returns 1 if any HitOverride slot is currently active. This mirrors Ikemen
// GO's `c.hoverIdx >= 0` semantics: the trigger fires whenever a HitOverride
// slot would catch the next incoming hit, NOT only after the override has
// actually triggered. Characters with armor/counter states (Akuma, Geese,
// KoldSpidey) gate their hit-reaction states on this trigger.
int isPlayerHitOverrideActive(DreamPlayer* tPlayer)
{
        if (!isGeneralPlayer(tPlayer)) return 0;
        PlayerHitOverrides* overrides = &tPlayer->mHitOverrides;
        ...
        int i;
        for (i = 0; i < 8; i++) {
                if (overrides->mHitOverrides[i].mIsActive) return 1;
        }
        return 0;
}
