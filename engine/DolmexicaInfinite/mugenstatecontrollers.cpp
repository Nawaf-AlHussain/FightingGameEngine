...
// Phase 2 — MapSet/MapAdd state controllers (Ikemen GO named float maps).
// Stores a named float value on the player's mMaps table. MapSet overwrites;
// MapAdd accumulates. Read back via the map(name) trigger.
typedef struct {
        DreamMugenAssignment* mName;
        DreamMugenAssignment* mValue;
} MapSetController;

static void parseMapSetController(DreamMugenStateController* tController, MugenDefScriptGroup* tGroup, DreamMugenStateControllerType tType) {
        MapSetController* e = (MapSetController*)allocMemoryOnMemoryStackOrMemory(sizeof(MapSetController));
        fetchAssignmentFromGroupAndReturnWhetherItExistsDefaultString("name", tGroup, &e->mName, "\"\"");
        fetchAssignmentFromGroupAndReturnWhetherItExistsDefaultString("value", tGroup, &e->mValue, "0");
        tController->mType = tType;
        tController->mData = e;
}

static void unloadMapSetController(DreamMugenStateController* tController) {
        MapSetController* e = (MapSetController*)tController->mData;
        destroyDreamMugenAssignment(e->mName);
        destroyDreamMugenAssignment(e->mValue);
        freeMemory(e);
}

static int handleMapSet(DreamMugenStateController* tController, DreamPlayer* tPlayer) {
        MapSetController* e = (MapSetController*)tController->mData;
        string name; evaluateDreamAssignmentAndReturnAsString(name, &e->mName, tPlayer);
        double value = evaluateDreamAssignmentAndReturnAsFloat(&e->mValue, tPlayer);
        setPlayerMap(tPlayer, name.c_str(), value);
        return 0;
}

static int handleMapAdd(DreamMugenStateController* tController, DreamPlayer* tPlayer) {
        MapSetController* e = (MapSetController*)tController->mData;
        string name; evaluateDreamAssignmentAndReturnAsString(name, &e->mName, tPlayer);
        double value = evaluateDreamAssignmentAndReturnAsFloat(&e->mValue, tPlayer);
        addPlayerMap(tPlayer, name.c_str(), value);
        return 0;
}
...
static void handleSingleSpecialAssert(DreamMugenAssignment** tAssignment, DreamPlayer* tPlayer) {
        ...
        // MUGEN AssertSpecial flags are case-insensitive. Characters use mixed
        // case: "NoWalk", "NoAutoTurn", "NoBG", "Intro", "Nostandguard", etc.
        // Lowercase the flag before comparison so all variants are recognized.
        ...
        // These call setters that store the flag in DreamPlayer. The flags
        // are queryable via isasserted(flag) but NOT enforced in gameplay
        // functions (enforcement caused movement bugs last time).
        else if ("noko" == flag) {
                setPlayerNoKOFlag(tPlayer);
        }
        else if ("animfreeze" == flag) {
                setPlayerAnimFreezeFlag(tPlayer);
        }
        else if ("nohitdamage" == flag) {
                setPlayerNoHitDamageFlag(tPlayer);
        }
        else if ("nofalldefenseup" == flag) {
                setPlayerNoFallDefenseUpFlag(tPlayer);
        }
        else if ("nocrouchmove" == flag) {
                setPlayerNoCrouchMoveFlag(tPlayer);
        }
        else if ("nostandmove" == flag) {
                setPlayerNoStandMoveFlag(tPlayer);
        }
        else if ("noairjump" == flag) {
                setPlayerNoAirJumpFlag(tPlayer);
        }
        else if ("nojump" == flag) {
                setPlayerNoJumpFlag(tPlayer);
        }
        else if ("noproj" == flag) {
                setPlayerNoProjFlag(tPlayer);
        }
        else if ("noclip" == flag) {
                setPlayerNoClipFlag(tPlayer);
        }
        else if ("nohardcodedkeys" == flag) {
                setPlayerNoHardcodedKeysFlag(tPlayer);
        }
        else if ("nobrake" == flag) {
                setPlayerNoBrakeFlag(tPlayer);
        }
        else if ("nompcost" == flag) {
                setPlayerNoMPCostFlag(tPlayer);
        }
        else if ("noguardbardisplay" == flag) {
                setPlayerNoGuardBarDisplayFlag(tPlayer);
        }
        else if ("nocombodisplay" == flag) {
                setPlayerNoComboDisplayFlag(tPlayer);
        }
        else if ("nofatigue" == flag) {
                setPlayerNoFatigueFlag(tPlayer);
        }
        else if ("noailevel" == flag) {
                setPlayerNoAILevelFlag(tPlayer);
        }
        else if ("nolifebaraction" == flag) {
                setPlayerNoLifebarActionFlag(tPlayer);
        }
        else if ("nopauserestore" == flag) {
                setPlayerNoPauseRestoreFlag(tPlayer);
        }
        else if ("nomp3" == flag) {
                setPlayerNoMP3Flag(tPlayer);
        }
        else if ("nopowerbardisplay" == flag) {
                setPlayerNoPowerBarDisplayFlag(tPlayer);
        }
        else if ("nohitbox" == flag) {
                setPlayerNoHitBoxFlag(tPlayer);
        }
        else if ("noguardbox" == flag) {
                setPlayerNoGuardBoxFlag(tPlayer);
        }
        else if ("noattackbox" == flag) {
                setPlayerNoAttackBoxFlag(tPlayer);
        }
        else if ("nodebuginfo" == flag) {
                setPlayerNoDebugInfoFlag(tPlayer);
        }
        else if ("nostatedisplay" == flag) {
                setPlayerNoStateDisplayFlag(tPlayer);
        }
        else if ("noface" == flag) {
                setPlayerNoFaceFlag(tPlayer);
        }
        else if ("noturn" == flag) {
                setPlayerNoTurnFlag(tPlayer);
        }
        else if ("nocounter" == flag) {
                setPlayerNoCounterFlag(tPlayer);
        }
        else if ("nodizzy" == flag) {
                setPlayerNoDizzyFlag(tPlayer);
        }
        else if ("noguarddamage" == flag) {
                setPlayerNoGuardDamageFlag(tPlayer);
        }
        else if ("noredlife" == flag) {
                setPlayerNoRedLifeFlag(tPlayer);
        }
        else if ("noguardspark" == flag) {
                setPlayerNoGuardSparkFlag(tPlayer);
        }
        else if ("nohitspark" == flag) {
                setPlayerNoHitSparkFlag(tPlayer);
        }
        else {
...
int mapAddHandleFunction(DreamMugenStateController* tController, DreamPlayer* tPlayer) { return handleMapAdd(tController, tPlayer); }
int mapSetHandleFunction(DreamMugenStateController* tController, DreamPlayer* tPlayer) { return handleMapSet(tController, tPlayer); }
...
int modifyHitDefHandleFunction(DreamMugenStateController* tController, DreamPlayer* tPlayer) { return handleHitDefinition(tController, tPlayer); }
int modifyProjectileHandleFunction(DreamMugenStateController* /*tController*/, DreamPlayer* /*tPlayer*/) { return 0; }
int modifyReversalDefHandleFunction(DreamMugenStateController* tController, DreamPlayer* tPlayer) { return handleReversalDefinition(tController, tPlayer); }
...
int rootVarAddHandleFunction(DreamMugenStateController* tController, DreamPlayer* tPlayer) { return handleAddingVariable(tController, tPlayer, getPlayerRoot(tPlayer)); }
int rootVarSetHandleFunction(DreamMugenStateController* tController, DreamPlayer* tPlayer) { return handleSettingVariable(tController, tPlayer, getPlayerRoot(tPlayer)); }
...
static void setupStateControllerHandlers() {
        ...
        gMugenStateControllerVariableHandler.mStateControllerHandlers[MUGEN_STATE_CONTROLLER_TYPE_MAP_SET] = mapSetHandleFunction;
        gMugenStateControllerVariableHandler.mStateControllerHandlers[MUGEN_STATE_CONTROLLER_TYPE_MAP_ADD] = mapAddHandleFunction;
        ...
        gMugenStateControllerVariableHandler.mStateControllerHandlers[MUGEN_STATE_CONTROLLER_TYPE_MODIFY_HIT_DEF] = modifyHitDefHandleFunction;
        gMugenStateControllerVariableHandler.mStateControllerHandlers[MUGEN_STATE_CONTROLLER_TYPE_MODIFY_REVERSAL_DEF] = modifyReversalDefHandleFunction;
        gMugenStateControllerVariableHandler.mStateControllerHandlers[MUGEN_STATE_CONTROLLER_TYPE_MODIFY_PROJECTILE] = modifyProjectileHandleFunction;
        ...
        gMugenStateControllerVariableHandler.mStateControllerHandlers[MUGEN_STATE_CONTROLLER_TYPE_ROOT_ADD_VARIABLE] = rootVarAddHandleFunction;
        gMugenStateControllerVariableHandler.mStateControllerHandlers[MUGEN_STATE_CONTROLLER_TYPE_SET_ROOT_VARIABLE] = rootVarSetHandleFunction;
...
void mapAddParseFunction(DreamMugenStateController* tController, MugenDefScriptGroup* tGroup) { parseMapSetController(tController, tGroup, MUGEN_STATE_CONTROLLER_TYPE_MAP_ADD); }
void mapSetParseFunction(DreamMugenStateController* tController, MugenDefScriptGroup* tGroup) { parseMapSetController(tController, tGroup, MUGEN_STATE_CONTROLLER_TYPE_MAP_SET); }
...
// Phase 3 Task 4 — ModifyHitDef/ModifyReversalDef/ModifyProjectile are clones
// of HitDef/ReversalDef/Projectile. They reuse the same parse function then
// override mType so the registered Modify* handler (which merges fields)
// runs instead of the replace-all handler. ModifyProjectile is currently a
// no-op stub (returns 0 without spawning a new projectile).
void modifyHitDefParseFunction(DreamMugenStateController* tController, MugenDefScriptGroup* tGroup) { parseHitDefinitionController(tController, tGroup); tController->mType = MUGEN_STATE_CONTROLLER_TYPE_MODIFY_HIT_DEF; }
void modifyProjectileParseFunction(DreamMugenStateController* tController, MugenDefScriptGroup* tGroup) { parseProjectileController(tController, tGroup); tController->mType = MUGEN_STATE_CONTROLLER_TYPE_MODIFY_PROJECTILE; }
void modifyReversalDefParseFunction(DreamMugenStateController* tController, MugenDefScriptGroup* tGroup) { parseReversalDefinitionController(tController, tGroup); tController->mType = MUGEN_STATE_CONTROLLER_TYPE_MODIFY_REVERSAL_DEF; }
...
void rootVarAddParseFunction(DreamMugenStateController* tController, MugenDefScriptGroup* tGroup) { parseVarSetController(tController, tGroup, MUGEN_STATE_CONTROLLER_TYPE_ROOT_ADD_VARIABLE); }
void rootVarSetParseFunction(DreamMugenStateController* tController, MugenDefScriptGroup* tGroup) { parseVarSetController(tController, tGroup, MUGEN_STATE_CONTROLLER_TYPE_SET_ROOT_VARIABLE); }
...
static void setupStateControllerParsers() {
        ...
        gMugenStateControllerVariableHandler.mStateControllerParsers["mapadd"] = mapAddParseFunction;
        gMugenStateControllerVariableHandler.mStateControllerParsers["mapset"] = mapSetParseFunction;
        gMugenStateControllerVariableHandler.mStateControllerParsers["modifyexplod"] = modifyExplodParseFunction;
        gMugenStateControllerVariableHandler.mStateControllerParsers["modifyhitdef"] = modifyHitDefParseFunction;
        gMugenStateControllerVariableHandler.mStateControllerParsers["modifyprojectile"] = modifyProjectileParseFunction;
        gMugenStateControllerVariableHandler.mStateControllerParsers["modifyreversaldef"] = modifyReversalDefParseFunction;
        ...
        gMugenStateControllerVariableHandler.mStateControllerParsers["rootvaradd"] = rootVarAddParseFunction;
        gMugenStateControllerVariableHandler.mStateControllerParsers["rootvarset"] = rootVarSetParseFunction;
...
void mapAddUnloadFunction(DreamMugenStateController* tController) { unloadMapSetController(tController); }
void mapSetUnloadFunction(DreamMugenStateController* tController) { unloadMapSetController(tController); }
void modifyExplodUnloadFunction(DreamMugenStateController* tController) { unloadModifyExplodController(tController); }
void modifyHitDefUnloadFunction(DreamMugenStateController* tController) { unloadHitDefinitionController(tController); }
void modifyProjectileUnloadFunction(DreamMugenStateController* tController) { unloadProjectileController(tController); }
void modifyReversalDefUnloadFunction(DreamMugenStateController* tController) { unloadReversalDefinitionController(tController); }
...
void rootVarSetUnloadFunction(DreamMugenStateController* tController) { unloadVarSetController(tController); }
void rootVarAddUnloadFunction(DreamMugenStateController* tController) { unloadVarSetController(tController); }
...
static void setupStateControllerUnloaders() {
        ...
        gMugenStateControllerVariableHandler.mStateControllerUnloaders[MUGEN_STATE_CONTROLLER_TYPE_MAP_SET] = mapSetUnloadFunction;
        gMugenStateControllerVariableHandler.mStateControllerUnloaders[MUGEN_STATE_CONTROLLER_TYPE_MAP_ADD] = mapAddUnloadFunction;
        ...
        gMugenStateControllerVariableHandler.mStateControllerUnloaders[MUGEN_STATE_CONTROLLER_TYPE_MODIFY_HIT_DEF] = modifyHitDefUnloadFunction;
        gMugenStateControllerVariableHandler.mStateControllerUnloaders[MUGEN_STATE_CONTROLLER_TYPE_MODIFY_REVERSAL_DEF] = modifyReversalDefUnloadFunction;
        gMugenStateControllerVariableHandler.mStateControllerUnloaders[MUGEN_STATE_CONTROLLER_TYPE_MODIFY_PROJECTILE] = modifyProjectileUnloadFunction;
        ...
        gMugenStateControllerVariableHandler.mStateControllerUnloaders[MUGEN_STATE_CONTROLLER_TYPE_ROOT_ADD_VARIABLE] = rootVarAddUnloadFunction;
        gMugenStateControllerVariableHandler.mStateControllerUnloaders[MUGEN_STATE_CONTROLLER_TYPE_SET_ROOT_VARIABLE] = rootVarSetUnloadFunction;
