...
static void drawSortedSprite(const DrawListSpriteElement* e) {
        ...
        case BLEND_TYPE_ADDITION:
                // WASM additive blending: use GL_ONE as destination factor (standard additive).
                // Previously used GL_DST_ALPHA (invisible in WebGL) and GL_CONSTANT_ALPHA
                // (caused black rectangle leak from glBlendColor global state).
                // GL_ONE makes both A and A1 render as full additive (minor visual
                // difference: A1 should be 50% dimmer, but this is acceptable and
                // doesn't break any characters).
                glBlendEquation(GL_FUNC_ADD);
...
static void drawSortedTruetype(const DrawListTruetypeElement* e) {
        ...
        for (i = 0; i < l;) {
                ...
                if (!surface) {
                        logWarningFormat("TTF_RenderText_Blended returned NULL: %s", TTF_GetError());
                        i = end + 1;
                        continue;
                }
