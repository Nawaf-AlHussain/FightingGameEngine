...
// =============================================================================
// Diagnostic exports — for debugging text rendering from JavaScript
// =============================================================================
// Implementations inside prism namespace, wrapped by extern "C" below.

int getMugenTextCountExportImpl() {
        return (int)gMugenTextHandler.mHandledTexts.size();
}

int getMugenTextVisibilityExportImpl(int tID) {
        if (!stl_map_contains(gMugenTextHandler.mHandledTexts, tID)) return -1;
        return gMugenTextHandler.mHandledTexts[tID].mIsVisible;
}

double getMugenTextXExportImpl(int tID) {
        if (!stl_map_contains(gMugenTextHandler.mHandledTexts, tID)) return -9999;
        return gMugenTextHandler.mHandledTexts[tID].mPosition.x;
}

double getMugenTextYExportImpl(int tID) {
        if (!stl_map_contains(gMugenTextHandler.mHandledTexts, tID)) return -9999;
        return gMugenTextHandler.mHandledTexts[tID].mPosition.y;
}

const char* getMugenTextContentExportImpl(int tID) {
        if (!stl_map_contains(gMugenTextHandler.mHandledTexts, tID)) return "";
        return gMugenTextHandler.mHandledTexts[tID].mDisplayText;
}

int getMugenFont0ValidExportImpl() {
        if (!stl_map_contains(gMugenFontData.mFonts, 0)) return -1;
        auto& font = gMugenFontData.mFonts[0];
        if (font.mType != MUGEN_FONT_TYPE_TRUETYPE) return -2;
        auto* ttFont = std::get_if<MugenTruetypeFont>(&font.mData);
        if (!ttFont) return 0;
        return (ttFont->mFont != NULL) ? 1 : 0;
}
...
int getMugenTextCountExport() {
        return prism::getMugenTextCountExportImpl();
}

int getMugenTextVisibilityExport(int tID) {
        return prism::getMugenTextVisibilityExportImpl(tID);
}

double getMugenTextXExport(int tID) {
        return prism::getMugenTextXExportImpl(tID);
}

double getMugenTextYExport(int tID) {
        return prism::getMugenTextYExportImpl(tID);
}

const char* getMugenTextContentExport(int tID) {
        return prism::getMugenTextContentExportImpl(tID);
}

int getMugenFont0ValidExport() {
        return prism::getMugenFont0ValidExportImpl();
}
