...
static struct {
        ...
        // SFF v2 sub-version: 0 = v2.00 (force index 0 transparent), 1 = v2.01 (respect per-entry alpha)
        // This is read from header.mVersion[2] and determines palette alpha handling.
        // v2.00 (MUGEN 1.0): index 0 is always transparent, all others are opaque
        // v2.01 (MUGEN 1.1): per-entry alpha is respected (any slot can be transparent)
        int mSffV2SubVersion = 0;
...
static Buffer processRawPalette2FixedAlpha(Buffer tRaw) {
        ...
        // Output ARGB (4 bytes per pixel) — matches processRawPalette2AlphaFromBuffer
        // format but with fixed alpha (index 0 transparent, all others opaque).
        // SFF v2 raw palette is stored as R,G,B,A per entry.
        // ARGB output: out[0]=A, out[1]=R, out[2]=G, out[3]=B
        ...
        for (i = 0; i < n; i++)
        {
                out[4 * i + 1] = raw[4 * i + 0]; // R from raw[0]
                out[4 * i + 2] = raw[4 * i + 1]; // G from raw[1]
                out[4 * i + 3] = raw[4 * i + 2]; // B from raw[2]
                out[4 * i + 0] = (i == 0) ? 0 : 255; // A: index 0 transparent, rest opaque
        }
...
static Buffer processRawPalette2AlphaFromBuffer(Buffer tRaw) {
        ...
        // Detect SFF v2.00 "flat-zero alpha" palettes. Some SFF v2.00 writers
        // (and many community tools that re-save palettes) write alpha=0 for
        // EVERY entry, including non-transparent colors. If we naively respect
        // those zeros, the whole sprite becomes invisible.
        //
        // Heuristic: if the alpha byte is 0 for every non-zero-index entry,
        // treat the file as v2.00-style and force alpha=255 for all non-zero
        // palette indices (index 0 stays transparent as the conventional
        // transparent color-key).
        int hasAnyNonZeroAlpha = 0;
        for (int i = 1; i < n; i++) {
                if (raw[4 * i + 3] != 0) {
                        hasAnyNonZeroAlpha = 1;
                        break;
                }
        }
        ...
        for (i = 0; i < n; i++) {
                ...
                out[4 * i + 3] = raw[4 * i + 2];
                if (i == 0) {
                        out[4 * i + 0] = 0; // index 0 always transparent
                } else if (hasAnyNonZeroAlpha) {
                        out[4 * i + 0] = raw[4 * i + 3]; // respect per-entry alpha (v2.01)
                } else {
                        out[4 * i + 0] = 255; // v2.00 flat-zero alpha: force opaque
                }
        }

        // JUS (Jump Ultimate Stars) characters use 32-color palettes instead of
        // the standard 256. The engine allocates a 256-entry palette buffer but
        // only fills `n` entries — entries [n..255] stay zeroed (transparent
        // black). If a sprite pixel references an index >= n (which can happen
        // with shared/effect sprites or palette remapping), it renders as
        // invisible. This is why Ultra Instinct Goku (a JUS character with
        // 32-color palettes) goes invisible during certain animations.
        //
        // Fix: fill unused entries [n..255] with the LAST valid color (index
        // n-1). This way, any out-of-range index renders as the closest valid
        // color rather than disappearing. This matches what Ikemen GO does
        // when it encounters out-of-range palette indices — it clamps to the
        // last available color.
        if (n >= 2) {
                uint8_t lastR = out[4 * (n - 1) + 1];
                uint8_t lastG = out[4 * (n - 1) + 2];
                uint8_t lastB = out[4 * (n - 1) + 3];
                uint8_t lastA = out[4 * (n - 1) + 0];
                for (i = n; i < 256; i++) {
                        out[4 * i + 0] = lastA;
                        out[4 * i + 1] = lastR;
                        out[4 * i + 2] = lastG;
...
{
        // Always use the alpha path (processRawPalette2AlphaFromBuffer) for ALL
        // SFF v2 files. The alpha values in the file are usually correct:
        // - v2.00: index 0 has alpha=0 (transparent), all others have alpha=255 (opaque)
        // - v2.01: per-entry alpha respected (any slot can be transparent)
        //
        // The alpha path reads raw[4*i+3] as the alpha value and forces index 0
        // to alpha=0. This works for both v2.00 and v2.01 in practice.
        //
        // The old heuristic ("raw[3] != 0x0") and the version-byte check both
        // caused issues:
        // - Heuristic: unreliable for files with non-standard alpha values
        // - Version check: v2.00 with FixedAlpha (RGB 3-byte) caused characters
        //   to turn black (3-byte palette treated as opaque in some code paths)
        //   or invisible (wrong byte order in ARGB conversion)
        //
        // By always using the alpha path, we let the file's own alpha values
        // determine transparency, with index 0 forced transparent as a safety net.
        (void)tRaw;
        return 1; // always use alpha path
}
...
// Two-pass palette loading for SFF v2:
// Pass 1: Read all palette headers and load palettes WITH data.
//         For linked palettes (mDataLength == 0), insert a placeholder.
// Pass 2: For each linked palette, copy the buffer from its link target
//         (palette index = mIndex field).
//
// This fixes invisibility for JUS and other characters that use palette
// linking. Goku_UI has 15 palettes with mDataLength=0 that link to other
// palettes via mIndex. Without this, those palettes get empty buffers and
// all sprites referencing them render invisible.
//
// tHasPalette: if true, an external palette was already inserted at
// mPalettes[0] before this function was called, so every SFF palette N
// lives at vector index N + 1. Link indices must be offset accordingly
// or linked palettes will copy the wrong source buffer.
static void loadPalettes2(SFFHeader2* tHeader, MugenSpriteFile* tDst, int tHasPalette) {
        int i;
        int paletteTotal = (int)tHeader->mPaletteTotal;
        const int offset = tHasPalette ? 1 : 0;

        // Pass 1: Read all palette headers
        gPrismMugenSpriteFileReaderData.mReader.mSeek(&gPrismMugenSpriteFileReaderData.mReader, tHeader->mPaletteOffset);
        std::vector<SFFPalette2> paletteHeaders(paletteTotal);
        for (i = 0; i < paletteTotal; i++) {
                gPrismMugenSpriteFileReaderData.mReader.mRead(&gPrismMugenSpriteFileReaderData.mReader, &paletteHeaders[i], sizeof(SFFPalette2));
        }

        // Pass 2: Load each palette. If it has data, load it. If it's linked
        // (mDataLength == 0), insert a placeholder for now — we'll fix it in
        // pass 2b after all palettes with data are loaded.
        for (i = 0; i < paletteTotal; i++) {
                SFFPalette2* pal = &paletteHeaders[i];

                if (pal->mDataLength > 0) {
                        // Has data — load it directly
                        uint32_t originalPosition = gPrismMugenSpriteFileReaderData.mReader.mGetCurrentOffset(&gPrismMugenSpriteFileReaderData.mReader);
                        gPrismMugenSpriteFileReaderData.mReader.mSeek(&gPrismMugenSpriteFileReaderData.mReader, tHeader->mLDataOffset + pal->mDataOffset);
                        Buffer rawPalette = gPrismMugenSpriteFileReaderData.mReader.mReadBufferReadOnly(&gPrismMugenSpriteFileReaderData.mReader, pal->mDataLength);
                        auto processedPalette = processRawPalette2(rawPalette);
                        freeBuffer(rawPalette);
                        gPrismMugenSpriteFileReaderData.mReader.mSeek(&gPrismMugenSpriteFileReaderData.mReader, originalPosition);

                        insertPaletteIntoMugenSpriteFile(tDst, processedPalette, pal->mGroupNo, pal->mItemNo);
                }
                else {
                        // Linked palette — insert a placeholder (opaque black ARGB)
                        Buffer placeholder = makeBufferOwned((uint8_t*)allocMemory(256 * 4), 256 * 4);
                        memset((uint8_t*)placeholder.mData, 0, 256 * 4);
                        insertPaletteIntoMugenSpriteFile(tDst, placeholder, pal->mGroupNo, pal->mItemNo);
                }
        }

        // Pass 2b: Now that all palettes with data are loaded, fix the linked
        // (placeholder) palettes by copying from their link targets.
        for (i = 0; i < paletteTotal; i++) {
                if (paletteHeaders[i].mDataLength == 0) {
                        int linkIdx = paletteHeaders[i].mIndex;
                        if (linkIdx >= 0 && linkIdx < paletteTotal && linkIdx != i && paletteHeaders[linkIdx].mDataLength > 0) {
                                // Copy the linked palette's buffer.
                                // SFF palette N is at vector index N + offset (external
                                // palette, if present, occupies slot 0).
                                auto& linkedPalette = tDst->mPalettes[linkIdx + offset];
                                auto& targetPalette = tDst->mPalettes[i + offset];
                                // Allocate new buffer and copy
                                uint8_t* copy = (uint8_t*)allocMemory(linkedPalette.mBuffer.mLength);
                                memcpy(copy, linkedPalette.mBuffer.mData, linkedPalette.mBuffer.mLength);
                                // Note: the old placeholder buffer leaks here, but this is
                                // a one-time load operation and the memory is from the
                                // engine's memory stack which gets freed on unload.
                                targetPalette.mBuffer = makeBufferOwned(copy, linkedPalette.mBuffer.mLength);
                        }
                }
        }
}
...
static MugenSpriteFile loadMugenSpriteFile2(int tHasPaletteFile, const char* tOptionalPaletteFile) {
        ...
        loadPalettes2(&header, &ret, tHasPaletteFile);
...
static MugenSpriteFile loadMugenSpriteFileGeneral(const char * tPath, int tHasPaletteFile, const char* tOptionalPaletteFile)
{
        ...
        if (header.mVersion[1] == 1 && header.mVersion[3] == 1) {
                ...
                // SFF v1.02 — treat as v2 (some tools mis-saved v2 files with these bytes)
                gPrismMugenSpriteFileReaderData.mSffV2SubVersion = header.mVersion[2];
                ret = loadMugenSpriteFile2(tHasPaletteFile, tOptionalPaletteFile);
        }
        else if (header.mVersion[1] == 0 && header.mVersion[3] == 2) {
                // SFF v2.00 (mVersion[2]==0) or v2.01 (mVersion[2]==1)
                // Store the sub-version for palette alpha handling
                gPrismMugenSpriteFileReaderData.mSffV2SubVersion = header.mVersion[2];
                ret = loadMugenSpriteFile2(tHasPaletteFile, tOptionalPaletteFile);
        }
        ...
        else if (header.mVersion[0] == 0 && header.mVersion[1] == 0 && header.mVersion[2] == 0 && header.mVersion[3] == 1) {
                // SFF v1.00 — treat as v1.01 (some old files have version 0,0,0,1)
                ret = loadMugenSpriteFile1(tHasPaletteFile, tOptionalPaletteFile);
        }
        else {
