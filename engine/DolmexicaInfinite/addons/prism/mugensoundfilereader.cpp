...
namespace prism {
        ...
        MugenSounds loadMugenSoundFile(const char* tPath)
        {
                ...
                // Handle empty/NULL path — return empty sounds instead of crashing
                if (!tPath || !tPath[0] || !isFile(tPath)) {
                        return ret;
                }

                initMugenSoundFileReader();
                gMugenSoundFileReaderData.mFileReader.mInit(&gMugenSoundFileReaderData.mFileReader, tPath);
                ret = loadMugenSoundFileWithReader();
        ...
        {
                if (!tSounds) return 0;  // NULL safety
        ...
        {
                setProfilingSectionMarkerCurrentFunction();
                if (!tSounds) return 0;  // NULL safety — no sound file loaded
                if (!hasMugenSound(tSounds, tGroup, tSample)) {
                        return 0;
                }
        ...
        {
                if (!tSounds) return 0;  // NULL safety
                if (!tSounds->mGroups.mBuckets) return 0;  // Uninitialized sound map
