...
namespace prism {
        ...
        typedef struct {
                uint32_t mRemoteButtons[MAXIMUM_CONTROLLER_AMOUNT]; // bitmask per controller (uint32_t, not uint8_t — UP=8, DOWN=9, START=10 overflow uint8_t)
        ...
        static void applyExternalInputOverlay() {
                for (int i = 0; i < MAXIMUM_CONTROLLER_AMOUNT; i++) {
                        ...
                        uint32_t mask = gExternalInput.mRemoteButtons[i]; // uint32_t, not uint8_t — UP/DOWN/START bits are >= 8
        ...
        // External input accessor (used by input_win.cpp and playerdefinition.cpp)
