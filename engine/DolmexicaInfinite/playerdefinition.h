...
struct DreamPlayer {
        ...
        // State-owner localcoord tracking (Phase 7.1)
        // Tracks whose state machine is currently running for this player.
        // When a player enters another player's state (custom states, throws),
        // this changes to the state owner's coordinateP. When returning to own
        // states, it changes back. Used for proper coordinate scaling.
        // See Ikemen Go's c.localscl for reference.
        int mStateOwnerCoordinateP;
        ...
        // ID of the player who last hit this player. Set in setPlayerHit().
        // Mirrors Ikemen GO's `c.hitVar.playerid`.
        int mLastAttackerID;
...
// Returns the number of air jumps the player has performed since leaving
// the ground. Exposed to CNS as the `airjumpcount` trigger so characters
// can gate multi-jump combos (e.g. "trigger1 = airjumpcount < 2"). Matches
// Ikemen GO's OC_ex2_airjumpcount (bytecode.go:984).
...
// isasserted(flag) trigger — returns 1 if the AssertSpecial flag is active
...
// Phase 1.2 — partner/enemy counting and partner(n) redirection.
// Computed from the actual roster (mAllPlayers) rather than hardcoded.
// In 1v1: getPlayerEnemyCount returns 1, getPlayerPartnerCount returns 0,
// getPlayerPartnerByIndex returns NULL. Will automatically produce correct
// results once simul/tag mode is added.
...
// Phase 1.4 — gethitvar(redirectId)/gethitvar(playerid).
// Returns the ID of the player who last hit p, or -1 if never hit.
int getPlayerLastAttackerID(DreamPlayer* p);

// Phase 1.4 — gethitvar(bindtime). Returns remaining bind time (frames).
...
// Global character draw scale multiplier (visual only — affects sprites,
// after-images, and explods at draw time). Defined in playerdefinition.cpp.
// 1.0 = original size, 0.7 = 70% size. Does NOT affect physics/collision/states.
...
int destroyPlayerWithFlags(DreamPlayer* tPlayer, int tRemoveExplods);
