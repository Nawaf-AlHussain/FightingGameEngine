...
static void startRound() {
        ...
        if (hasLoadedPlayerSprites()) {
                // Diagnostic logging: capture state before state 5900 transition
                for (int i = 0; i < 2; i++) {
                        DreamPlayer* p = getRootPlayer(i);
                        if (p) {
                                logg("[LOCALCOORD_DIAG] startRound player:");
                                logInteger(p->mRootID);
                                logString(" coordinateP=");
                                logInteger(getPlayerCoordinateP(p));
                                logString(" toCameraScale=");
                                logInteger(getPlayerToCameraScale(p) * 1000);
                                logString(" stateNo=");
                                logInteger(getPlayerState(p));
                                logString(" alive=");
                                logInteger(isPlayerAlive(p));
                        }
                }
