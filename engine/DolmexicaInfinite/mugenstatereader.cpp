...
static void loadMugenConstantsVelocityData(DreamMugenConstantsVelocityData* tVelocityData, MugenDefScript* tScript) {
        ...
        // MUGEN 1.1 / Ikemen GO: when jump.back, jump.fwd, airjump.back, airjump.fwd,
        // or airjump.neu are defined as single numbers (X only, no Y), the Y velocity
        // should inherit from jump.neu's Y component. The engine's parser interprets
        // single-number vectors as Vector2D(x, 0) — Y=0. Without this fix, characters
        // like Goku_UI (JUS) that define "jump.back = -4" get Y=0, making backward
        // jumps have no upward velocity.
        if (tVelocityData->mJumpNeutral.y != 0) {
                if (tVelocityData->mJumpBackward.y == 0) tVelocityData->mJumpBackward.y = tVelocityData->mJumpNeutral.y;
                if (tVelocityData->mJumpForward.y == 0) tVelocityData->mJumpForward.y = tVelocityData->mJumpNeutral.y;
                if (tVelocityData->mAirJumpNeutral.y == 0) tVelocityData->mAirJumpNeutral.y = tVelocityData->mJumpNeutral.y;
                if (tVelocityData->mAirJumpBackward.y == 0) tVelocityData->mAirJumpBackward.y = tVelocityData->mJumpNeutral.y;
                if (tVelocityData->mAirJumpForward.y == 0) tVelocityData->mAirJumpForward.y = tVelocityData->mJumpNeutral.y;
        }
