...
#include "playerdefinition.h" // for getGlobalCharacterDrawScale()
...
static void getExplodSpritesAnimationsScale(Explod* e, MugenSpriteFile** tSprites, MugenAnimation** tAnimation, double* tBaseScale) {
        if (e->mIsInFightDefFile) {
                ...
                *tAnimation = getPlayerAnimation(e->mPlayer, e->mAnimationNumber);
                // Apply global character draw scale to character-relative explods (hit sparks,
                // after-image effects, etc.) so they match the smaller character size.
                *tBaseScale = getPlayerToCameraScale(e->mPlayer) * getGlobalCharacterDrawScale();
        }
}
