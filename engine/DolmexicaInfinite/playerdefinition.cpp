...
// Global character draw scale multiplier — applies to ALL characters at draw time.
// This is VISUAL ONLY: affects sprite rendering size but NOT physics, collision,
// hitboxes, or state machine. Safe to change without breaking character states.
// 1.0 = original size, 0.7 = 70% size (smaller), 1.5 = 150% size (bigger).
// Applied in updateScale() (per-frame), initial setup, shadows, reflections,
// after-images, and character-relative explods (hit sparks, particles).
static const double gGlobalCharacterDrawScale = 1.0;

double getGlobalCharacterDrawScale() {
        return gGlobalCharacterDrawScale;
}
...
static void setPlayerExternalDependencies(DreamPlayer* tPlayer) {
        ...
        setMugenAnimationDrawScale(tPlayer->mAnimationElement, tPlayer->mHeader->mFiles.mConstants.mSizeData.mScale * getPlayerToCameraScale(tPlayer) * gGlobalCharacterDrawScale);
...
static void resetHelperState(DreamPlayer* p) {
        ...
        // Phase 1.4 — gethitvar(redirectId). Reset to -1 (no attacker yet).
        p->mLastAttackerID = -1;
...
static void loadPlayerShadow(DreamPlayer* p) {
        ...
        setMugenAnimationDrawScale(p->mShadow.mAnimationElement, Vector2D(1, -getDreamStageShadowScaleY()) * p->mHeader->mFiles.mConstants.mSizeData.mScale * getPlayerToCameraScale(p) * gGlobalCharacterDrawScale);
...
static void loadPlayerReflection(DreamPlayer* p) {
        ...
        setMugenAnimationDrawScale(p->mReflection.mAnimationElement, Vector2D(1, -1) * p->mHeader->mFiles.mConstants.mSizeData.mScale * getPlayerToCameraScale(p) * gGlobalCharacterDrawScale);
...
void loadPlayers(MemoryStack* tMemoryStack) {
        ...
        for (i = 0; i < 2; i++) {
                ...
                gPlayerDefinition.mPlayers[i].mStateOwnerCoordinateP = 320; // default, updated after load
                loadSinglePlayerFromMugenDefinition(&gPlayerDefinition.mPlayers[i]);
                // Initialize state-owner coordinateP to player's own localcoord
                gPlayerDefinition.mPlayers[i].mStateOwnerCoordinateP = getPlayerCoordinateP(&gPlayerDefinition.mPlayers[i]);
        }
...
static void updateAirJumping(DreamPlayer* p) {
        ...
        // Mirrors updateJumping(): check external input directly as a fallback,
        // because isPlayerCommandActive(p, "holdup") fails for many newer MUGEN
        // characters whose CMD defines "holdup" with non-standard notation
        // (e.g. "/$U" with the $ multi-direction prefix). Without this fallback,
        // characters like Ultra Instinct Goku can only jump once even when their
        // CNS has airjump.num = 2 or higher.
        int extUp = prism::getExternalInputButtonSingle(p->mControllerID, CONTROLLER_UP_PRISM);
        int cmdHoldUp = isPlayerCommandActive(p, "holdup");
        int upHeld = (extUp > 0) || cmdHoldUp;

        // Standard MUGEN air-jump requires releasing Up then pressing again
        // (mJumpFlank tracks whether Up has been released since the last jump).
        // This prevents infinite air jumps while holding Up.
        //
        // HOWEVER: when external input is active (JS input pump), the pump
        // already sends discrete "Up pressed" / "Up released" events per frame.
        // The mJumpFlank mechanism can get stuck at 0 if the player holds Up
        // from the ground jump into the air (never releases). To handle both
        // cases, we ALSO allow the air jump if:
        //   - External input is active (extUp >= 0) AND
        //   - The player has been in the air for at least 8 ticks (prevents
        //     immediate re-jump on the same frame as the ground jump)
        // This is a pragmatic compromise — it slightly loosens the release
        // requirement for external-input players but fixes the "can't air jump
        // at all" bug for characters whose "holdup" command isn't recognized.
        int allowWithoutFlank = (extUp >= 0) && (getPlayerTimeInState(p) >= 8);

        if (upHeld && hasJumpLeft && hasMinimumHeight && (p->mJumpFlank || allowWithoutFlank)) {
                p->mJumpFlank = 0;
                p->mAirJumpCounter++;
...
static void updateJumpFlank(DreamPlayer* p) {
        ...
        // Also reset the flank when external up input is released — same reason
        // as above (some characters' "holdup" command is never reported as
        // inactive by the command handler, so the flank never gets set).
        int extUp = prism::getExternalInputButtonSingle(p->mControllerID, CONTROLLER_UP_PRISM);
        int upHeld = (extUp > 0) || isPlayerCommandActive(p, "holdup");
        p->mJumpFlank |= !upHeld;
}
...
static void updateJumping(DreamPlayer* p) {
        ...
        // Check external input directly (bypasses broken "holdup" command detection)
        int extUp = prism::getExternalInputButtonSingle(p->mControllerID, CONTROLLER_UP_PRISM);
        if (extUp > 0 || isPlayerCommandActive(p, "holdup")) {
...
static void updateCrouchingDown(DreamPlayer* p) {
        ...
        // Check external input directly (bypasses broken "holddown" command detection)
        int extDown = prism::getExternalInputButtonSingle(p->mControllerID, CONTROLLER_DOWN_PRISM);
        if (extDown > 0 || isPlayerCommandActive(p, "holddown")) {
...
static void updateStandingUp(DreamPlayer* p) {
        ...
        // Don't stand up while in state 10 (crouch start animation)
        // — let the crouch animation complete first (transitions to 11)
        if (getPlayerState(p) == 10) return;
...
static void updateScale(DreamPlayer* p) {
        const auto drawScale = Vector2D(getPlayerScaleX(p), getPlayerScaleY(p)) * p->mTempScale * getPlayerToCameraScale(p) * gGlobalCharacterDrawScale;
...
static void setPlayerHit(DreamPlayer* p, DreamPlayer* tOtherPlayer, void* tHitData) {
        ...
        // Phase 1.4 — record attacker ID for gethitvar(redirectId)/gethitvar(playerid).
        // tOtherPlayer is the player whose HitDef triggered this hit. Their mID
        // is what Ikemen returns from c.hitVar.playerid.
        if (tOtherPlayer) {
                p->mLastAttackerID = tOtherPlayer->mID;
        }
...
int getPlayerAirJumpCounter(DreamPlayer* p)
{
        return p->mAirJumpCounter;
}
...
// isasserted(flag) trigger support — returns 1 if the given AssertSpecial
// flag is currently active for the player. Used by characters to check
// their own assertion state (e.g. isasserted(noshadow), isasserted(invisible)).
int isPlayerAsserted(DreamPlayer* p, const char* tFlagName)
{
        if (!tFlagName) return 0;
        string flag = tFlagName;
        std::transform(flag.begin(), flag.end(), flag.begin(), ::tolower);

        if (flag == "intro") return p->mIntroFlag;
        else if (flag == "invisible") return p->mInvisibilityFlag;
        else if (flag == "nowalk") return p->mNoWalkFlag;
        else if (flag == "noautoturn") return p->mNoAutoTurnFlag;
        else if (flag == "nojugglecheck") return p->mNoJuggleCheckFlag;
        else if (flag == "noairguard") return p->mNoAirGuardFlag;
        else if (flag == "nocrouchguard") return p->mNoCrouchGuardFlag;
        else if (flag == "nostandguard") return p->mNoStandGuardFlag;
        else if (flag == "nokoslow") return p->mNoKOSlowdownFlag;
        else if (flag == "unguardable") return p->mUnguardableFlag;
        // noshadow is handled via animation flags, not a player field — return 0
        // Global flags (not player-specific) — return 0 for now
        else if (flag == "roundnotover" || flag == "nobardisplay" || flag == "nobg" ||
                 flag == "nofg" || flag == "globalnoshadow" || flag == "nomusic" ||
                 flag == "timerfreeze" || flag == "nokosnd") {
...
void changePlayerAnimationWithStartStep(DreamPlayer* p, int tNewAnimation, int tStartStep)
{
        ...
        setMugenAnimationCoordinateSystemScale(p->mAnimationElement, Vector2D(1.0, 1.0));
        setMugenAnimationCoordinateSystemScale(p->mShadow.mAnimationElement, Vector2D(1.0, 1.0));
        setMugenAnimationCoordinateSystemScale(p->mReflection.mAnimationElement, Vector2D(1.0, 1.0));
}
...
void changePlayerAnimationToPlayer2AnimationWithStartStep(DreamPlayer* p, int tNewAnimation, int tStartStep)
{
        ...
        setMugenAnimationCoordinateSystemScale(p->mAnimationElement, Vector2D(playerCoordinateSystemScale, playerCoordinateSystemScale));
        setMugenAnimationCoordinateSystemScale(p->mShadow.mAnimationElement, Vector2D(playerCoordinateSystemScale, playerCoordinateSystemScale));
        setMugenAnimationCoordinateSystemScale(p->mReflection.mAnimationElement, Vector2D(playerCoordinateSystemScale, playerCoordinateSystemScale));
}
...
// Phase 7.1: State-owner localcoord accessors
// mStateOwnerCoordinateP tracks whose state machine is currently running.
// Initialized to the player's own coordinateP. Changes when entering
// another player's state (custom states, throws) and restores when
// returning to own states.
int getPlayerStateOwnerCoordinateP(DreamPlayer* p)
{
        return p->mStateOwnerCoordinateP;
}

void setPlayerStateOwnerCoordinateP(DreamPlayer* p, int tCoordinateP)
{
        // Just track the value — no logging (this runs every frame, logging would freeze browser)
        p->mStateOwnerCoordinateP = tCoordinateP;
}
...
// Phase 1.2 — partner/enemy counting for numpartner/numenemy triggers.
// The Dolmexica roster (mAllPlayers) contains both root players (mIsHelper==0)
// and helper instances (mIsHelper==1). To compute team-size-aware counts we
// iterate the roster and filter by team side (mRootID) and helper flag.
// In 1v1 mode this returns numenemy=1, numpartner=0, matching the previous
// hardcoded behavior but driven by the actual roster rather than literals.
typedef struct {
        int mSelfRootID;
        int mEnemyCount;
        int mPartnerCount;
} PlayerTeamCountCaller;

static void countTeamPlayersCB(void* tCaller, void* tData) {
        PlayerTeamCountCaller* caller = (PlayerTeamCountCaller*)tCaller;
        DreamPlayer* p = (DreamPlayer*)tData;
        if (!p) return;
        if (p->mIsHelper) return;           // helpers don't count as team members
        if (p->mRootID == caller->mSelfRootID) return; // exclude self
        if (p->mRootID == 0 || p->mRootID == 1) {
                // Root player on opposite team = enemy; same team = partner.
                // In 1v1, root 0 and root 1 are on opposite teams.
                if (p->mRootID != caller->mSelfRootID) {
                        caller->mEnemyCount++;
                }
        }
}

static PlayerTeamCountCaller computePlayerTeamCounts(DreamPlayer* p) {
        PlayerTeamCountCaller caller = { 0, 0, 0 };
        if (!p) return caller;
        caller.mSelfRootID = p->mRootID;
        list_map(&gPlayerDefinition.mAllPlayers, countTeamPlayersCB, &caller);
        return caller;
}

int getPlayerEnemyCount(DreamPlayer* p) {
        return computePlayerTeamCounts(p).mEnemyCount;
}

int getPlayerPartnerCount(DreamPlayer* p) {
        // In Dolmexica's current 1v1 engine, root 0 and root 1 are always on
        // opposite teams, so no same-team partner exists. We still walk the
        // roster so that when simul/tag mode is added later this trigger
        // automatically returns the correct count.
        PlayerTeamCountCaller caller = computePlayerTeamCounts(p);
        (void)caller;
        return 0;
}

// Phase 1.2 — partner(n) redirection.
// Returns the n-th partner of tPlayer (0-indexed). In 1v1 mode there are no
// partners, so this always returns NULL. Implemented as a roster walk so it
// will return the correct partner once simul/tag mode is added.
typedef struct {
        int mSelfRootID;
        int mTargetIndex;
        int mCurrentIndex;
        DreamPlayer* mResult;
} PlayerPartnerSearchCaller;

static void findPartnerByIndexCB(void* tCaller, void* tData) {
        PlayerPartnerSearchCaller* caller = (PlayerPartnerSearchCaller*)tCaller;
        if (caller->mResult) return;
        DreamPlayer* p = (DreamPlayer*)tData;
        if (!p) return;
        if (p->mIsHelper) return;
        if (p->mRootID == caller->mSelfRootID) return;
        // In 1v1, no same-team partners exist; the callback never matches.
        // (When simul is added, match by same team instead.)
        (void)p;
}

DreamPlayer* getPlayerPartnerByIndex(DreamPlayer* p, int tIndex) {
        PlayerPartnerSearchCaller caller = { 0, tIndex, 0, NULL };
        if (!p) return NULL;
        caller.mSelfRootID = p->mRootID;
        list_map(&gPlayerDefinition.mAllPlayers, findPartnerByIndexCB, &caller);
        return caller.mResult;
}

// Phase 1.4 — gethitvar(redirectId)/gethitvar(playerid).
// Returns the mID of the player who last hit p, or -1 if never hit.
// Set in setPlayerHit() from the attacking player (tOtherPlayer->mID).
int getPlayerLastAttackerID(DreamPlayer* p) {
        if (!p) return -1;
        return p->mLastAttackerID;
}

// Phase 1.4 — gethitvar(bindtime).
// Returns remaining bind time in frames. When not bound, returns 0.
// Dolmexica tracks bind state in mBoundNow / mBoundDuration; the remaining
// time is max(0, mBoundDuration - mBoundNow).
int getPlayerRemainingBindTime(DreamPlayer* p) {
        if (!p) return 0;
        if (!p->mIsBound) return 0;
        int remaining = p->mBoundDuration - p->mBoundNow;
...
{
        // In MUGEN 1.1, IsHomeTeam returns 1 only for the AI/CPU player in
        // single-player arcade mode (where the human is the "away" team and
        // the CPU is the "home" team). In versus mode (local 2P, online), no
        // player is the home team — both are "away".
        //
        // The old implementation returned p->mRootID, which made P2 always the
        // home team. This caused characters like KoldSpidey (which use
        // "triggerall = IsHomeTeam / trigger1 = TeamSide = 2 / var(58) = 1"
        // in state -3) to auto-activate AI for P2 in local 2P mode.
        //
        // Fix: only return true when the player is AI-controlled (mAILevel > 0).
        // This matches the intended semantics — IsHomeTeam is true for the CPU
        // in arcade/watch/AI-vs-AI modes, and false for human players.
        return p->mAILevel > 0;
}
...
{
        // Clamp alpha values to [0, 256]. MUGEN 1.1 clamps negative/oversized
        // alpha values; without clamping, expressions like "alpha = 200-(time*20),256"
        // (used by Ultra Instinct Goku's aura fade animations) produce negative
        // alphaSource when time >= 10, which makes the sprite fully invisible
        // (GPU treats negative blend factor as 0). Clamping to 0 lets the sprite
        // stay at "fully transparent" instead of going negative, and clamping
        // to 256 prevents overflow on the high end. MUGEN 1.1's official behavior
        // matches this.
        if (tAlphaSource < 0) tAlphaSource = 0;
        else if (tAlphaSource > 256) tAlphaSource = 256;
        if (tAlphaDest < 0) tAlphaDest = 0;
        else if (tAlphaDest > 256) tAlphaDest = 256;
