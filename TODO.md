...
70. ✅ **HitOverride ignored when HitDef sets p2stateno (armor characters getting thrown)** — In `setPlayerHitStatesPlayer()` (`playerdefinition.cpp`), the `tHasMatchingHitOverride` check was nested *inside* the `p2stateno == -1` branch, so any HitDef that sets `p2stateno` (e.g. throws) skipped the HitOverride check entirely and redirected the victim straight into the attacker's throw-victim state, even with an active, matching HitOverride. Fixed by checking `tHasMatchingHitOverride` first, unconditionally, before falling through to the `p2stateno == -1` ground/air/crouch/lying logic and then the `p2getp1state`/plain-`p2stateno` branches. `setPlayerHitStatesNonPlayer()` (helpers/projectiles) already checked HitOverride correctly and wasn't touched — it doesn't handle `p2stateno` for helpers at all, which may be a separate, unconfirmed gap. **Source-traced only, not built or tested in a running build — treat with suspicion until verified against an armor character with a HitOverride vs. a throw.**
71. ✅ **OOB read after animation end** — `loadNextStepAndReturnIfShouldBeRemoved`'s non-looping "keep alive" branch (added by the animelemtime fix, item 67) only matched `mStep == vector_size(...)` on the *first* overrun; every subsequent call to this function — which keeps happening, since the whole point of "kept alive" is that it doesn't stop — incremented `mStep` again with nothing to catch it, so it grew by 1 forever, unbounded, for as long as the finished animation stayed alive. Two call sites (`getMugenAnimationElementFromTimeOffset` and its `...Loop` helper) read `e->mStep` directly via `vector_get()` with no clamping — unlike `getCurrentAnimationStep()`'s safe `min(mStep, count-1)` read — so this was a real out-of-bounds memory read, not a theoretical one, once a finished animation had been alive for more than an instant. Fixed by pinning `mStep` at the last valid index every time instead of letting it drift, and changed the guard from `==` to `>=` for defense in depth. **Source-traced only, not built or tested.**
72. ✅ **`facep2` statedef flag parsed but never applied** — `mDoesFacePlayer2` / `MUGEN_STATE_PROPERTY_FACE_PLAYER_2_INFO` were correctly parsed by `mugenstatereader.cpp` and stored on `DreamMugenState`, but `changeDreamHandledStateMachineState()` (`mugenstatehandler.cpp`) — the function that applies every *other* statedef-level property (`velset`, `ctrl`, `poweradd`, `juggle`, `anim`, `sprpriority`, the three persistence flags) on state entry — never checked this one's flag at all, so `facep2 = 1` in a statedef silently did nothing. Fixed by adding the same `hasPrismFlag(...)` gated block as the others, reusing the existing `turnPlayerTowardsOtherPlayer()` (already used by `updateAutoTurn()` and the round-win pose in `gamelogic.cpp`) rather than duplicating turn logic. **Source-traced only, not built or tested.**

### Session 9 — External-analysis spot-check (found via an independent review, not this project's own audits)

73. ✅ **`strlen()` of an uninitialized buffer in player-redirect resolution** (2 occurrences) — `getRegularPlayerFromFirstVectorPartOrNullIfNonexistant()` and its story-mode sibling `getStoryPlayerFromFirstVectorPartOrNullIfNonexistant()` (both `mugenassignmentevaluator.cpp`) declare a local `char text[100]` buffer, then immediately call `strlen(text)` as a bounds check — **before anything has written to `text`**. This reads uninitialized stack memory (undefined behavior) and, more importantly, provides zero actual overflow protection, since the check was never against the string actually being copied in. The evident intent was to bounds-check the source string (`rawVar->mName`) before `strcpy`-ing it into the fixed 100-byte buffer. Fixed both occurrences: `strlen(text)` → `strlen(rawVar->mName)`. This path is hit by any raw player-redirect trigger (`p1`, `p2`, `parent`, `root`, `target`, etc. in their bare, non-array form). In practice this likely "worked" most of the time because leftover stack contents often contain a null byte within 100 bytes, masking the bug — not something to rely on, especially under Emscripten/WASM's stack layout, which doesn't give the same guarantees a native debug build's zeroed stack might. **Source-traced only, not built or tested.**

**Note on how this was found:** this bug wasn't in any of this project's own audits/deep-dives — it
surfaced from an independent third-party review of the repo (via ChatGPT, per the user) that was
then verified against actual current source before acting on it, per this project's standing rule
of not trusting any analysis - including this one - without checking it against source directly.
Worth periodically getting a fresh, differently-biased set of eyes on the codebase this way.

**⚠️ Deployment note (found while verifying this item):** the repo's latest commit has a newer
build (`dev-1787656270`) than what's actually live on Vercel (`dev-1787635702` as of this
writing) — see `HANDOFF.md`'s "Critical build fact" section. Worth checking before assuming any
of these fixes are visible to a user testing the live site.

**None of items 64-70 have been built and regression-tested together.** See `HANDOFF.md` Section 3
