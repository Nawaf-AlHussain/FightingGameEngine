...
| `facep2` statedef flag parsed but never applied | ✅ Fixed | Correctly parsed into `mDoesFacePlayer2`/`MUGEN_STATE_PROPERTY_FACE_PLAYER_2_INFO`, but `changeDreamHandledStateMachineState()` — which applies every *other* statedef-level property (`velset`, `ctrl`, `poweradd`, `juggle`, `anim`, `sprpriority`, the persistence flags) on state entry — never checked this one. Fixed by adding the same pattern, reusing the existing `turnPlayerTowardsOtherPlayer()`. **Source-traced only, not built or run.** |
...
6. Medium/low items in `TODO.md`'s "Known Issues" section — physics-before-
